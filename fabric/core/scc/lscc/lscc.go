...
func (lscc *LifeCycleSysCC) executeDeploy(
	...
	fmt.Println("jyp: check signature (check policy when instantiate chaincode) (disable)", m_file, m_line, time.Now().String()[:26])
...
func (lscc *LifeCycleSysCC) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	case INSTALL:
		...
		_, m_file, m_line, _ := runtime.Caller(0)
		fmt.Println("jyp: check signature (check policy when install chaincode) (disable)", m_file, m_line, time.Now().String()[:26])
