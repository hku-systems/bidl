...
func (handler *Handler) serialSend(msg *pb.ChaincodeMessage) error {
	fmt.Printf("jyp: GetState 4\n")
	//jyp: send
	st := time.Now()
	chaincodeLogger.Debugf("jyp: sleep start at %s shim/handler.go\n", st.String()[:27])
	fmt.Printf("jyp: sleep start at %s shim/handler.go\n", st.String()[:27])
	handler.serialLock.Lock()
	defer handler.serialLock.Unlock()
	ed := time.Since(st) / time.Millisecond
	fmt.Printf("jyp: sleep end at %s (%dms) shim/handler.go\n", time.Now().String()[:27], ed)
