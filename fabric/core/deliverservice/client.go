...
func (bc *broadcastClient) Recv() (*orderer.DeliverResponse, error) {
	//jyp: called from (core/deliverservice/blocksprovider/blocksprovider.go:DeliverBlocks)
...
func (bc *broadcastClient) connect() error {
	...
	abc, err := bc.createClient(conn).Deliver(ctx)  // jyp: gossip: create Deliver
	if err != nil {
