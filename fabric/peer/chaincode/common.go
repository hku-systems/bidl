...
func getChaincodeSpec(cmd *cobra.Command) (*pb.ChaincodeSpec, error) {
	...
	fmt.Println("jyp: get chaincode: ", chaincodeName, chaincodePath, chaincodeCtorJSON, m_file, m_line, time.Now().String()[:26])
	return spec, nil
}

func chaincodeInvokeOrQuery(cmd *cobra.Command, invoke bool, cf *ChaincodeCmdFactory) (err error) {
	_, m_file, m_line, _ := runtime.Caller(0)
	fmt.Println("jyp: send transaction; breakdown(send Tx)", m_file, m_line, time.Now().String()[:26])
	...
	if invoke {
		...
		fmt.Println("jyp: receive result; breakdown(receive result)", ms_file, ms_line, time.Now().String()[:26])
		...
		_, ms_file, ms_line, _ := runtime.Caller(0)
		fmt.Println("jyp: receive transaction result; breakdown(receive result)", ms_file, ms_line, time.Now().String()[:26])
	}
	return nil
}
...
func ChaincodeInvokeOrQuery(
	...
	_, m_file, m_line, _ := runtime.Caller(0)
	fmt.Println("jyp: create proposal", m_file, m_line, time.Now().String()[:26])
	...
	_, mt_file, mt_line, _ := runtime.Caller(0)
	fmt.Println("jyp: sign tx here", mt_file, mt_line, time.Now().String()[:26])
	...
	_, ms_file, ms_line, _ := runtime.Caller(0)
	fmt.Println("jyp: receive proposal result; breakdown(receive proposal result)", ms_file, ms_line, time.Now().String()[:26])
	...
	if invoke {
		if proposalResp != nil {
			...
			_, m_file, m_line, _ := runtime.Caller(0)
			fmt.Println("jyp: sign simulated response", m_file, m_line, time.Now().String()[:26])
			...
			_, ms_file, ms_line, _ := runtime.Caller(0)
			fmt.Println("jyp: send transaction for ordering; breakdown(ordering)", ms_file, ms_line, time.Now().String()[:26])
			...
			if dg != nil && ctx != nil {
				...
				_, m_file, m_line, _ := runtime.Caller(0)
				fmt.Println("jyp: wait for result", m_file, m_line, time.Now().String()[:26])
				err = dg.Wait(ctx)
				if err != nil {
					return nil, err
				}
...
func (dg *deliverGroup) ClientWait(dc *deliverClient) {
	...
	for {
		...
		case *pb.DeliverResponse_FilteredBlock:
			fmt.Println("jyp: event: receive block from peer, check tx's status")
