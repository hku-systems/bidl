...
func CreateSignedEnvelopeWithTLSBinding(txType common.HeaderType, channelID string, signer crypto.LocalSigner, dataMsg proto.Message, msgVersion int32, epoch uint64, tlsCertHash []byte) (*common.Envelope, error) {
	...
	fmt.Println("jyp: sign envelope here (disabled)", m_file, m_line, time.Now().String()[:26])
	// if signer != nil {
...
func CreateSignedTx(proposal *peer.Proposal, signer msp.SigningIdentity, resps ...*peer.ProposalResponse) (*common.Envelope, error) {
	...
	_, m_file, m_line, _ := runtime.Caller(0)
	fmt.Println("jyp: remove signer consistency checking", m_file, m_line, time.Now().String()[:26])
	...
	fmt.Println("jyp: sign tx(with endorsements) (disabled)", ms_file, ms_line, time.Now().String()[:26])
	// sign the payload
	// sig, err := signer.Sign(paylBytes)
	// if err != nil {
	// 	return nil, err
...
func GetSignedProposal(prop *peer.Proposal, signer msp.SigningIdentity) (*peer.SignedProposal, error) {
	...
	_, m_file, m_line, _ := runtime.Caller(0)
	fmt.Println("jyp: signed here (disabled)", m_file, m_line, time.Now().String()[:26])
	...
	// signature, err := signer.Sign(propBytes)  // disable
	// if err != nil {
	// 	return nil, err
	// }
