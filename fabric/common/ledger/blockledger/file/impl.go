...
func (fl *FileLedger) Append(block *cb.Block) error {
	fmt.Println("jyp: append a new block to ledger", time.Now().String()[:26])
	err := fl.blockStore.AddBlock(block) // jyp: call (common/ledger/blkstorage/fsblkstorage/blockfile_mgr.go)
	if err == nil {
		close(fl.signal)
