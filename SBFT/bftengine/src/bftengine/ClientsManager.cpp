...
bool ClientsManager::noPendingAndRequestCanBecomePending(NodeIdType clientId, ReqId reqSeqNum) const {
  ...
  if (c.currentPendingRequest != 0) {
    // printf("client %d has pending request before %ld\n", clientId, reqSeqNum);
    return false;  // if has pending request

  }

  if (reqSeqNum <= c.lastSeqNumberOfReply) {
    // printf("client %d already executed %ld/%ld\n", clientId, reqSeqNum, c.lastSeqNumberOfReply);
