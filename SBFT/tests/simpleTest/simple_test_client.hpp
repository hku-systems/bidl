...
class SimpleTestClient {
 ...
 public:
  ...
  bool run() {
    ...
    // ddbug: wait for TCP connections completed.
    while(!client->ready()) {
      sleep(1);
    }
    ...
    for (uint32_t i = 1; i <= cp.numOfOperations; i++) {
      ...
      if (i % readMod == 0) {
...
#ifdef SBFT_PAYLOAD_SIZE
        // Prepare request parameters.
        const uint32_t kRequestLength = SBFT_PAYLOAD_SIZE; 
        const char requestBuffer[kRequestLength] = "SBFT_PAYLOAD_READ";
        const char* rawRequestBuffer = reinterpret_cast<const char*>(requestBuffer);
        const uint32_t rawRequestLength = sizeof(char) * kRequestLength;
#else
        // Prepare request parameters.
        ...
        // Prepare request parameters.
        const uint32_t kRequestLength = SBFT_PAYLOAD_SIZE; 
        const char requestBuffer[kRequestLength] = "SBFT_PAYLOAD_WRITE";
        const char* rawRequestBuffer = reinterpret_cast<const char*>(requestBuffer);
        const uint32_t rawRequestLength = sizeof(char) * kRequestLength;
#else
        // Prepare request parameters.
      ...
      // "bEnchmark timestamp(ms) latency(us)"
      timestamps.push_back( "bEnchmark " + std::to_string((uint64_t)(end / 1e3)) + " " + std::to_string(elapsedMicro));
