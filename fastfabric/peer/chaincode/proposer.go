...
func CreateProposers(conn, client int, nodes []Node, logger *log.Logger) *Proposers {
	...
	for _, node := range nodes {
		...
		for j := 0; j < conn; j++ {
			row[j] = CreateProposer(node.Addr, logger)
		}
		ps = append(ps, row)
	}
	...
	for i := 0; i < len(config.Endorsers); i++ {
		...
		for k := 0; k < config.ClientPerConn; k++ {
			for j := 0; j < config.NumOfConn; j++ {
				...
				go ps.workers[i][j].Start(signed[i], processed, done, 1)
			}
		}
	}
}
...
func CreateProposer(addr string, logger *log.Logger) *Proposer {
	endorser, err := CreateEndorserClient(addr)
	if err != nil {
		panic(err)
	}
...
func (p *Proposer) Start(signed, processed chan *Elements, done <-chan struct{}, threshold int) {
	...
	for {
		...
		case s := <-signed:
			...
			if interval > 2 {
				// fmt.Printf("jyp: send to endorser %.3f tps %d %f\n", float64(cnt)/interval, cnt, interval)
				st = ed
				cnt = 0
			}
			...
			// fmt.Printf("jyp: breakdown endorsement %dms\n", time.Since(st)/time.Millisecond)
			s.lock.Lock()
...
func CreateBroadcasters(conn int, orderer Node, logger *log.Logger) Broadcasters {
	...
	for i := 0; i < conn; i++ {
		bs[i] = CreateBroadcaster(orderer.Addr, logger)
	}
...
func CreateBroadcaster(addr string, logger *log.Logger) *Broadcaster {
	client, err := CreateBroadcastClient(addr)
	if err != nil {
		panic(err)
	}
...
func (b *Broadcaster) Start(envs <-chan *Elements, done <-chan struct{}) {
	...
	for {
		...
		case e := <-envs:
			...
			interval := ed.Sub(st).Seconds()
			if interval > 2 {
				fmt.Printf("jyp: send tx to orderer: %.3f tps %d %f\n", float64(cnt)/interval, cnt, interval)
				st = ed
				cnt = 0
			}
			if err != nil {
