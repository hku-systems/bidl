...
func serve(args []string) error {
	...
	listenAddr := viper.GetString("peer.listenAddress")  // jyp: what's this? not found in config file.
	_, m_file, m_line, _ := runtime.Caller(0)
	fmt.Println("jyp: listenAddr=", listenAddr, m_file, m_line, time.Now().String()[:26])

	serverConfig, err := peer.GetServerConfig()  // jyp: tls related config
	if err != nil {
	...
	fmt.Println("jyp: register grpc deliver server")
	...
	fmt.Println("jyp: TOOD: how to init plugin")
	...
	fmt.Println("jyp: register grpc Endorser server")
...
func registerChaincodeSupport(grpcServer *comm.GRPCServer, ccEndpoint string, ca tlsgen.CA, packageProvider *persistence.PackageProvider, aclProvider aclmgmt.ACLProvider, pr *platforms.Registry) (*chaincode.ChaincodeSupport, ccprovider.ChaincodeProvider, *scc.Provider) {
	...
	ipRegistry := inproccontroller.NewRegistry() // jyp: cc call (core/container/inproccontroller/inproccontroller.go )
	...
	chaincodeSupport := chaincode.NewChaincodeSupport(
		...
		lsccInst,
		aclProvider,
		container.NewVMController(map[string]container.VMProvider{ // jyp: cc call (core/container/controller.go)
			// jyp: cc "DOCKER"
			dockercontroller.ContainerType: dockercontroller.NewProvider( // jyp: cc call (core/container/dockercontroller/dockercontroller.go)
				viper.GetString("peer.id"),
				viper.GetString("peer.networkId"),
			),
			inproccontroller.ContainerType: ipRegistry, // jyp: cc "SYSTEM
		}),
		sccp,
		pr,
		peer.DefaultSupport,
...
func startAdminServer(peerListenAddr string, peerServer *grpc.Server) {
	...
	fmt.Println("jyp: register grpc Admin server")
