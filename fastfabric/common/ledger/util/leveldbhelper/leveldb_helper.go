...
func (dbInst *DB) Get(key []byte) ([]byte, error) {
	...
	// }
	value, ok := dbInst.db.Load(string(key))
	if !ok {
		return nil, nil
	}
...
func (dbInst *DB) Put(key []byte, value []byte, sync bool) error {
	...
	// }
	dbInst.db.Store(string(key), string(value))
	return nil
}
...
func (dbInst *DB) GetIterator(startKey []byte, endKey []byte) iterator.Iterator {
	...
	db, _ := leveldb.OpenFile("jyp_temp_db", nil)
	iter := db.NewIterator(nil, nil)
	return iter // dbInst.db.NewIterator(&goleveldbutil.Range{Start: startKey, Limit: endKey}, dbInst.readOpts)

}

func (dbInst *DB) GetCache(startKey []byte, endKey []byte) map[string]string {
	var res map[string]string
	dbInst.db.Range(func(k, v interface{}) bool {
