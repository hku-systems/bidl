...
func (w *blockfileWriter) append(b []byte, sync bool) error {
	// fileStat, err := w.file.Stat()
	// fmt.Printf("jyp: write data to file %s at %s\n", fileStat.Name(), time.Now().String()[:26])
	// st := time.Now()
	// defer func() {
	// fmt.Printf("jyp: wite done at %s in %dms\n", time.Now().String()[:26], time.Since(st)/time.Millisecond)
	// }()
	...
	if sync {
		return w.file.Sync() //jyp: write to disk now
	}
	return nil
}
