...
func (h *Handler) serialSend(msg *pb.ChaincodeMessage) error {
	// jyp: send
	// fmt.Printf("jyp: send msg to chaincode container?\n")
	// st := time.Now()
	// fmt.Printf("jyp: sleep start at %s chaincode/handler.go\n", st.String()[:27])
	h.serialLock.Lock()
	// ed := time.Since(st) / time.Millisecond
	// fmt.Printf("jyp: sleep end at %s (%dms) chaincode/handler.go\n", time.Now().String()[:27], ed)
...
func (h *Handler) HandleRegister(msg *pb.ChaincodeMessage) {
	...
	err = h.Registry.Register(h) // jyp: cc
	if err != nil {
