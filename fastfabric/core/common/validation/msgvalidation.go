...
func ValidateProposalMessage(signedProp *pb.SignedProposal) (*pb.Proposal, *common.Header, *pb.ChaincodeHeaderExtension, error) {
	...
	// fmt.Println("jyp: endorser validate the signature of proposal (disabled)", m_file, m_line, time.Now().String()[:26])
	//n err = checkSignatureFromCreator(shdr.Creator, signedProp.Signature, signedProp.ProposalBytes, chdr.ChannelId)
	// if err != nil {
...
func checkSignatureFromCreator(creatorBytes []byte, sig []byte, msg []byte, ChainID string) error {
	...
	// validate the signature
	_, m_file, m_line, _ := runtime.Caller(0)
	fmt.Println("jyp: verify msg signature (disabled)", m_file, m_line, time.Now().String()[:26])
...
func validateEndorserTransaction(data []byte, hdr *common.Header) error {
	...
	for _, act := range tx.Actions {
		...
		fmt.Println("jyp: may need remove in the future (TODO)")
...
func ValidateTransaction(e *common.Envelope, c channelconfig.ApplicationCapabilities) (*common.Payload, pb.TxValidationCode) {
	...
	// _, m_file, m_line, _ := runtime.Caller(0)
	// fmt.Println("jyp: endorser validate the signature of transaction (disabled)", m_file, m_line, time.Now().String()[:26])
