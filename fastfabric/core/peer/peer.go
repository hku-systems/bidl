...
func (cs *chainSupport) Apply(configtx *common.ConfigEnvelope) error {
	...
	// fmt.Println("jyp: apply user chaincode", m_file, m_line, time.Now().String()[:26])
	err := cs.ConfigtxValidator().Validate(configtx)
	if err != nil {
		_, m_file, m_line, _ := runtime.Caller(0)
		fmt.Println("jyp: ERROR apply user chaincode", m_file, m_line, time.Now().String()[:26])
		return err
	}
...
func createChain(cid string, ledger ledger.PeerLedger, cb *common.Block, ccp ccprovider.ChaincodeProvider, sccp sysccprovider.SystemChaincodeProvider, pm txvalidator.PluginMapper) error {
	...
	validator := txvalidator.NewTxValidator(cid, vcs, sccp, pm) // jyp: call (core/committer/txvalidator/validator.go)
