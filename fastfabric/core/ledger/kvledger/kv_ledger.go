...
func (l *kvLedger) CommitWithPvtData(pvtdataAndBlock *ledger.BlockAndPvtData) error {
	// jyp: called from (gossip/privdata/coordinator.go:StoreBlock)
	var err error
	block := pvtdataAndBlock.Block
	blockNo := pvtdataAndBlock.Block.Header.Number

	startStateValidation := time.Now()
	logger.Debugf("[%s] qiji:Validating state for block [%d]", l.ledgerID, blockNo)
	...
	logger.Debugf("[%s] qiji:Committing block [%d] to storage", l.ledgerID, blockNo)
	// fmt.Printf("jyp: wait for lock %s\n", time.Now().String()[:27])
	// st := time.Now()
	l.blockAPIsRWLock.Lock()
	// fmt.Printf("jyp: wait (%dms) for block at %s\n", time.Since(st)/time.Millisecond, time.Now().String()[:27])
	// st = time.Now()
	defer func() {
		// fmt.Printf("jyp: hold lock for (%dms) from %s until %s\n", time.Since(st)/time.Millisecond, st.String()[:27], time.Now().String()[:27])
		l.blockAPIsRWLock.Unlock()
	}()
	//jyp: call (core/ledger/ledgerstorage/store.go)
