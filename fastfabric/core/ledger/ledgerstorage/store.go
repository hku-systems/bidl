...
type Store struct {
	...
	blockQueue chan *ledger.BlockAndPvtData
}
...
func (p *Provider) Open(ledgerid string) (*Store, error) {
	...
	store := &Store{blockStore, pvtdataStore, &sync.RWMutex{}, make(chan *ledger.BlockAndPvtData, 100000)}
...
func (s *Store) Init(btlPolicy pvtdatapolicy.BTLPolicy) {
	...
	go func() {
		for  {
			select {
			case block := <- s.blockQueue:
				s.doCommitWithPvtData(block)
			}
		}
	}()
}
func (s *Store) doCommitWithPvtData(blockAndPvtdata *ledger.BlockAndPvtData) error {
	...
	//jyp: call (common/ledger/blkstorage/fsblkstorage/fs_blockstore.go)
...
func (s *Store) CommitWithPvtData(blockAndPvtdata *ledger.BlockAndPvtData) error {
	s.blockQueue <- blockAndPvtdata
	// TODO: jyp, only for experiment
	return nil
}
