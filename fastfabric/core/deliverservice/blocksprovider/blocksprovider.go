...
func (b *blocksProviderImpl) DeliverBlocks() {
	// fmt.Println("jyp: pull blocks, called from (core/deliverservice/deliveryclient.go")
	errorStatusCounter := 0
	statusCounter := 0
	defer b.client.Close()
	for !b.isDone() {
		logger.Infof("qiji: blockprovider recv")
		msg, err := b.client.Recv() //jyp: call (core/deliverservice/client.go)
		logger.Infof("qiji: blockprovider recv finish")
		if err != nil {
		...
		case *orderer.DeliverResponse_Status:
			...
			logger.Infof("qiji: blockprovider -2")
			...
			logger.Infof("qiji: blockprovider -1")
			...
			logger.Infof("qiji: blockprovider 0")
			continue
		case *orderer.DeliverResponse_Block:
			// fmt.Printf("jyp: leading peer receive block from orderer at %s\n", time.Now().String()[:27])
			errorStatusCounter = 0
			statusCounter = 0
			...
			logger.Infof("qiji: blockprovider 3")
