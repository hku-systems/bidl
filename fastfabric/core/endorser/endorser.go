...
func (e *Endorser) callChaincode(txParams *ccprovider.TransactionParams, version string, input *pb.ChaincodeInput, cid *pb.ChaincodeID) (*pb.Response, *pb.ChaincodeEvent, error) {
	...
	// jyp: call (core/endorser/support.go)
...
func (e *Endorser) SimulateProposal(txParams *ccprovider.TransactionParams, cid *pb.ChaincodeID) (ccprovider.ChaincodeDefinition, *pb.Response, []byte, *pb.ChaincodeEvent, error) {
	...
	if txParams.TXSimulator != nil {
		// jyp: call (core/ledger/kvledger/txmgmt/txmgr/lockbasedtxmgr/lockbased_tx_simulator.go)
...
func (e *Endorser) endorseProposal(_ context.Context, chainID string, txid string, signedProp *pb.SignedProposal, proposal *pb.Proposal, response *pb.Response, simRes []byte, event *pb.ChaincodeEvent, visibility []byte, ccid *pb.ChaincodeID, txsim ledger.TxSimulator, cd ccprovider.ChaincodeDefinition) (*pb.ProposalResponse, error) {
	...
	// fmt.Println("jyp: TODO, endorse with plugin?", m_file, m_line, time.Now().String()[:26])
...
func (e *Endorser) preProcess(signedProp *pb.SignedProposal) (*validateResult, error) {
	...
	prop, hdr, hdrExt, err := validation.ValidateProposalMessage(signedProp) // jyp: including verify signature
	...
	if err != nil {
		fmt.Println("jyp: debug signature header 1")
		vr.resp = &pb.ProposalResponse{Response: &pb.Response{Status: 500, Message: err.Error()}}
		return vr, err
	}
...
func (e *Endorser) ProcessProposal(ctx context.Context, signedProp *pb.SignedProposal) (*pb.ProposalResponse, error) {
	...
	//_, m_file, m_line, _ := runtime.Caller(0)
	//fmt.Println("jyp: endorser process proposal", m_file, m_line, time.Now().String()[:26])
	addr := util.ExtractRemoteAddress(ctx)
	// fmt.Println("Entering: request from", addr, time.Now().String()[:27])
	endorserLogger.Debug("Entering: request from", addr)
	defer func(time_st time.Time) {
		endorserLogger.Debug("Exit: request from", addr)
		// fmt.Println("Exit: request from", addr, time.Now().String()[:27])
		//fmt.Printf("jyp: endorsement done in (%dms) for txid=%s\n", time.Since(time_st)/time.Millisecond, txid)
	}(time.Now())
	...
	if err != nil {
		// _, m_file, m_line, _ := runtime.Caller(0)
		// fmt.Println("jyp: simulate error", m_file, m_line, time.Now().String()[:26])
		return &pb.ProposalResponse{Response: &pb.Response{Status: 500, Message: err.Error()}}, nil
	}
	// qiji: error message
	if res != nil {
	...
	if chainID == "" {
		// _, m_file, m_line, _ := runtime.Caller(0)
		// fmt.Println("jyp: chainless proposals dont't have to be endorsed", m_file, m_line, time.Now().String()[:26])
