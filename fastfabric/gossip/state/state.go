...
func (s *GossipStateProviderImpl) listen() {
	...
	for {
		...
		case msg := <-s.gossipChan:
			// fmt.Println("jyp: Received new message via gossip channel")
	...
	for {
		...
		case <-s.payloads.Ready():
			...
			for payload := s.payloads.Pop(); payload != nil; payload = s.payloads.Pop() {
				...
				logger.Infof("qiji: deliverPayloads 1")
				...
				logger.Infof("qiji: deliverPayloads 2")
				...
				logger.Infof("qiji: deliverPayloads 3")
				// _, ms_file, ms_line, _ := runtime.Caller(0)
				// fmt.Println("jyp: breakdown(peer receive block to vscc and commit)", ms_file, ms_line, time.Now().String()[:26])
				// fmt.Println("jyp: gossip, ready to comit block;")
				...
				logger.Infof("qiji: deliverPayloads 4")
			}
		case <-s.stopCh:
			s.stopCh <- struct{}{}
...
func (s *GossipStateProviderImpl) addPayload(payload *proto.Payload, blockingMode bool) error {
	...
	// fmt.Println("jyp: gossip, block received")
...
func (s *GossipStateProviderImpl) commitBlock(block *common.Block, pvtData util.PvtDataCollections) error {
	...
	// fmt.Println("jyp: gossip, call StoreBlock (gossip/privdata/coordinator.go)")
