...
func NewCoordinator(support Support, selfSignedData common.SignedData) Coordinator {
	// jyp: called from (gossip/service/gossip_service.go: InitializeChannel)
...
func (c *coordinator) StoreBlock(block *common.Block, privateDataSets util.PvtDataCollections) error {
	// jyp: called from gossip/state/state.go:commitBlock
	if block.Data == nil {
	...
	logger.Debugf("[%s] Received block [%d] from buffer", c.ChainID, block.Header.Number)

	logger.Infof("[%s] Validating block [%d]", c.ChainID, block.Header.Number)

	// _, m_file, m_line, _ := runtime.Caller(0)
	// fmt.Println("jyp: validate block (-2)", m_file, m_line, time.Now().String()[:26])
	...
	// qiji: set ownedRWsets to a empty map
	...
	logger.Debugf("qiji: StoreBlock 2")
	// qiji: remove test
	/*startPull := time.Now()
	...
	logger.Debugf("qiji: StoreBlock 3")
	...
	logger.Debugf("qiji: StoreBlock 4")

	// populate the private RWSets passed to the ledger
	for seqInBlock, nsRWS := range ownedRWsets.bySeqsInBlock() {
		rwsets := nsRWS.toRWSet()
		logger.Infof("[%s] Added %d namespace private write sets for block [%d], tran [%d]", c.ChainID, len(rwsets.NsPvtRwset), block.Header.Number, seqInBlock)
	...
	// fmt.Println("jyp: bottleneck? call CommitWithPvtData (core/ledger/kvledger/lv_ledger.go)")
	logger.Debugf("qiji: MVCC -1")
