...
func (g *gossipServiceImpl) InitializeChannel(chainID string, endpoints []string, support Support) {
	//jyp: called from (core/peer/peer.go:creatChain)
	g.lock.Lock()
	defer g.lock.Unlock()
	...
	coordinator := privdata2.NewCoordinator(privdata2.Support{   // jyp: call (gossip/privdata/coordinator.go)
		ChainID:         chainID,
...
func (g *gossipServiceImpl) AddPayload(chainID string, payload *gproto.Payload) error {
	...
	return g.chains[chainID].AddPayload(payload) // jyp: call (gossip/state/state.go)
}
