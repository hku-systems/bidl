...
public final class TOMLayer extends Thread implements RequestReceiver {
        ...
        if (controller.getStaticConf().getBatchTimeout() > -1) {
            ...
            batchTimer.scheduleAtFixedRate(new TimerTask() {
                ...
                public void run() {
                    ...
                    if (clientsManager.havePendingRequests() && (System.currentTimeMillis() - lastRequest) >= controller
                            .getStaticConf().getBatchTimeout()) {
    ...
    public void requestReceived(TOMMessage msg) {
        ...
        if (readOnly) {
            ...
            logger.info("Received TOMMessage from client " + msg.getSender() + " with sequence number "
                    + msg.getSequence() + " for session " + msg.getSession());
    ...
    public void run() {
        ...
        while (doWork) {
            ...
            messagesLock.lock();
            if (!clientsManager.havePendingRequests() || (controller.getStaticConf().getBatchTimeout() > -1
    ...
    public TOMMessage[] checkProposedValue(byte[] proposedValue, boolean addToClientManager) {
        // if (BidlFrontend.txMap.containsKey(key)) {

        // }
        try {
            logger.debug("bidl: Checking proposedValue, " + " length: " + proposedValue.length);
            ...
            logger.debug("bidl: deserialiseRequests, " + " num: " + requests.length);
            // logger.debug("bidl: deserialiseRequests, " + " length: " +
            // requests[0].getContent().length + " content: "
            // + Arrays.toString(requests[0].getContent()));
            ...
            if (addToClientManager) {
                ...
                logger.debug("bidl: in checkProposedValue, there are {} TOMMessages in the proposed value",
                        requests.length);
                for (TOMMessage request : requests) {
                    logger.trace("bidl: deserialiseRequests, " + " length: " + request.getContent().length
                            + " content: " + Arrays.toString(request.getContent()));

                    // boolean hashOK = checkTxHash(request.getContent());
                    // if (!hashOK) {
                    // return null;
                    // }
    ...
    private boolean checkTxHash(byte[] proposedValue) {
        // first 4 bytes are length, last 4 bytes is a integer indicates the length of
        // signature (0)
        int len = 0;
        byte[] hash = null;
        for (int i = 4; i < proposedValue.length - 4; i++) {
            ...
            if (len == 32) {
                ...
                if (!BidlFrontend.txMap.containsKey(hashStr)) {
                    logger.info("bidl: I don't hold the transaction, hash {}", Arrays.toString(hash));
                    return false;
                }
                len = 0;
            }
