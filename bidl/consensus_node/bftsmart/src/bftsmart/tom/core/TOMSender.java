...
public abstract class TOMSender implements ReplyReceiver, Closeable, AutoCloseable {
	...
	// only send assembled block to myself
	public void TOMe(TOMMessage sm, int myID) {
		System.out.println("bidl: in TOMe myID: " + myID);
		int[] target = new int[1];
		target[0] = myID;
		cs.send(useSignatures, target, sm);
	}

	public void TOMe(byte[] m, int myID, int reqId, int operationId, TOMMessageType reqType) {
		System.out.println("bidl: in TOMe2, myID: " + myID + " me: " + me);
		int[] target = new int[1];
		target[0] = myID;
